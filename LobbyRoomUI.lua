...
local function populateCreateRoomFrame()
	...
	confirmButton.MouseButton1Click:Connect(function()
		...
		print("Sending create room request to server with name: " .. settings.roomName .. " and mode: " .. gameMode)
...
local function populateSoloFrame()
	...
	local storyButton = createButton("Story Mode", 1)
	storyButton.Parent = soloFrame
	storyButton.Position = UDim2.new(0.5, -100, 0.4, 0)
	storyButton.MouseButton1Click:Connect(function()
		print("Client requesting to start a solo game in Story mode.")
		lobbyRemote:FireServer("startSoloGame", { gameMode = "Story" })
	end)

	local endlessButton = createButton("Endless Mode", 2)
	endlessButton.Parent = soloFrame
	endlessButton.Position = UDim2.new(0.5, -100, 0.6, 0)
	endlessButton.MouseButton1Click:Connect(function()
		print("Client requesting to start a solo game in Endless mode.")
		lobbyRemote:FireServer("startSoloGame", { gameMode = "Endless" })
	end)
end
