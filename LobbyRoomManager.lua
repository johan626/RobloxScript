...
-- matchmakingQueues[playerCount][gameMode] = {player1, player2, ...}
...
local function handleCreateRoom(hostPlayer, settings, initialPlayers)
	...
	local gameMode = settings.gameMode or "Story" -- Default to Story mode
	...
	local newRoom = {
		...
		RoomId = roomId,
		GameMode = gameMode -- Store the game mode
	}
...
local function handleCancelMatchmaking(player)
	for playerCount, modes in pairs(matchmakingQueues) do
		for mode, queue in pairs(modes) do
			for i, p in ipairs(queue) do
				if p == player then
					table.remove(queue, i)
					print(string.format("Player %s cancelled matchmaking for %d players in %s mode.", player.Name, playerCount, mode))
...
local function checkForFullQueue(playerCount, gameMode)
	local queue = matchmakingQueues[playerCount] and matchmakingQueues[playerCount][gameMode]
	if not queue then return end

	if #queue >= playerCount then
		print(string.format("Full queue found for %d players in %s mode. Creating room.", playerCount, gameMode))
		...
		local settings = { maxPlayers = playerCount, isPrivate = true, gameMode = gameMode }
...
local function handleStartMatchmaking(player, data)
	...
	if not matchmakingQueues[playerCount][gameMode] then
		matchmakingQueues[playerCount][gameMode] = {}
	end

	table.insert(matchmakingQueues[playerCount][gameMode], player)
	print(string.format("Player %s entered matchmaking for %d players in %s mode.", player.Name, playerCount, gameMode))

	lobbyRemote:FireClient(player, "matchmakingStarted")
	checkForFullQueue(playerCount, gameMode)
end
...
local function teleportPlayersToAct1(playersToTeleport, gameMode)
	...
	teleportOptions:SetTeleportData({
		gameMode = gameMode or "Story" -- Default to Story if not provided
	})
	...
	if not teleportSuccess then
		...
		print("Successfully initiated teleport for ", #playersToTeleport, " players to the private server with mode: " .. (gameMode or "Story"))
	end
end
	...
	task.spawn(function()
		for i = 5, 0, -1 do
			...
			if i == 0 then
				...
				teleportPlayersToAct1(playersToUpdate, currentRoom.GameMode)
			end
...
lobbyRemote.OnServerEvent:Connect(function(player, action, data)
	...
	elseif action == "startSoloGame" then
		...
		print("Player", player.Name, "is starting a solo game in mode: " .. gameMode)
		teleportPlayersToAct1({player}, gameMode)
