...
local gameModeInitialized = false

local function initializeGameMode(player)
	if gameModeInitialized then return end

	local joinData = player:GetJoinData()
	local teleportData = joinData and joinData.TeleportData

	if teleportData and teleportData.gameMode then
		gameMode = teleportData.gameMode
		gameModeInitialized = true
		print("Game Mode initialized from JoinData. Mode: " .. gameMode)
	else
		-- Fallback for players joining directly or if data is missing
		gameModeInitialized = true
		print("No JoinData found for game mode. Defaulting to Story.")
	end
end
...
playerAddedConnection = Players.PlayerAdded:Connect(function(player)
	initializeGameMode(player)
	-- Disconnect after the first player joins to avoid re-initializing
	if gameModeInitialized and playerAddedConnection then
...
if #Players:GetPlayers() > 0 then
	initializeGameMode(Players:GetPlayers()[1])
	if gameModeInitialized and playerAddedConnection then
		playerAddedConnection:Disconnect()
		playerAddedConnection = nil
	end
...
if not RemoteEvents:FindFirstChild("GameModeUpdateEvent") then
	local event = Instance.new("RemoteEvent")
	event.Name = "GameModeUpdateEvent"
	event.Parent = RemoteEvents
end
...
local function startGameLoop()
	...
	task.spawn(function()
		while true do
			...
			chamsApplied = false
			ClearChams()
			local isBossWave = SpawnerModule.SpawnWave(zombiesToSpawn, wave, activePlayers, gameMode)
...
game.Players.PlayerAdded:Connect(function(player)
	...
	DataStoreManager:OnPlayerDataLoaded(player, function(playerData)
		...
		local gameModeUpdateEvent = RemoteEvents:FindFirstChild("GameModeUpdateEvent")
		if gameModeUpdateEvent then
			gameModeUpdateEvent:FireClient(player, gameMode)
		end
	end)
end)
